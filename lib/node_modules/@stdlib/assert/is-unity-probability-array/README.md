...
```javascript
const isUnityProbabilityArray = require( '@stdlib/assert/is-unity-probability-array' );
```
...
let bool = isUnityProbabilityArray( [ 0.25, 0.5, 0.25 ] );
// returns true
...
-   Summation of finite-precision floating-point numbers often has numerical error. For example,
    ...
    ```javascript
    const arr = [ 0.1, 0.2, 0.1, 0.1, 0.2, 0.2, 0.1 ]; // => 1.0
    let sum = 0.0;
...
let arr = [ 0.0, 1.0 ];
let bool = isUnityProbabilityArray( arr );
// returns true
