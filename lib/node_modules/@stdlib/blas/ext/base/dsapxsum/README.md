...
```javascript
const dsapxsum = require( '@stdlib/blas/ext/base/dsapxsum' );
```
...
const v = dsapxsum.ndarray( 3, 5.0, x, 1, 0 );
// returns 16.0
```
...
const v = dsapxsum.ndarray( 4, 5.0, x, 2, 1 );
// returns 25.0
```
...
const v = dsapxsum( x.length, 5.0, x, 1 );
console.log( v );
```
