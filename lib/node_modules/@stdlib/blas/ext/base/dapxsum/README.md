...
```javascript
const dapxsum = require( '@stdlib/blas/ext/base/dapxsum' );
```
...
const v = dapxsum.ndarray( N, 5.0, x, 1, 0 );
// returns 16.0
```
...
const v = dapxsum.ndarray( 4, 5.0, x, 2, 1 );
// returns 25.0
```
...
const v = dapxsum( x.length, 5.0, x, 1 );
console.log( v );
```
