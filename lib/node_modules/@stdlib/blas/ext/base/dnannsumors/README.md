...
```javascript
const dnannsumors = require( '@stdlib/blas/ext/base/dnannsumors' );
```
...
const v = dnannsumors( x.length, x, 1, out, 1 );
...
const v = dnannsumors( 4, x, 2, out, 1 );
...
const v = dnannsumors( 4, x1, 2, out1, 1 );
...
const v = dnannsumors.ndarray( x.length, x, 1, 0, out, 1, 0 );
// returns <Float64Array>[ 1.0, 3 ]
```
...
const v = dnannsumors.ndarray( 4, x, 2, 1, out, 2, 1 );
