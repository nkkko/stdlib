...
```javascript
const dnannsumpw = require( '@stdlib/blas/ext/base/dnannsumpw' );
```
...
const v = dnannsumpw( x.length, x, 1, out, 1 );
...
const v = dnannsumpw( 4, x, 2, out, 1 );
...
const v = dnannsumpw( 4, x1, 2, out1, 1 );
...
const v = dnannsumpw.ndarray( x.length, x, 1, 0, out, 1, 0 );
// returns <Float64Array>[ 1.0, 3 ]
```
...
const v = dnannsumpw.ndarray( 4, x, 2, 1, out, 2, 1 );
