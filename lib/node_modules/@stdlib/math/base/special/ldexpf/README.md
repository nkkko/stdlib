...
```javascript
const ldexpf = require( '@stdlib/math/base/special/ldexpf' );
```
...
```javascript
let x = ldexpf( 0.5, 3 ); // => 0.5 * 2^3 = 0.5 * 8
// returns 4.0
...
const frac = linspace( 0.0, 100.0, 10 );
const exp = discreteUniform( 100, 0, 10 );
