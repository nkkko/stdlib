...
```javascript
const deepSet = require( '@stdlib/utils/deep-set' );
```
...
const bool = deepSet( obj, 'a.b.c', 'beep' );
// returns true
...
let bool = deepSet( obj, 'a.b.c', 'woot' );
// returns true
...
const bool = deepSet( arr, '1.a.0.x', 25 );
// returns true
...
const bool = deepSet( obj, [ 'a', 'b', 'c' ], 'beep' ); // obj => { 'a': { 'b': { 'c': 'beep' } } }
// returns true
```
...
const bool = deepSet( obj, 'a.b.c', set ); // obj => { 'a': { 'b': { 'c': 'ddddd' } } }
// returns true
```
...
const bool = deepSet( obj, 'a.e.c', 'boop', {
    'create': true
});
// returns true
...
```javascript
const dset = deepSet.factory( 'a/b/c', {
    'create': true,
    'sep': '/'
});
```
...
for ( let i = 0; i < data.length; i++ ) {
    keys[ 0 ] = i;
    const bool = deepSet( data, keys, set );
    if ( !bool ) {
